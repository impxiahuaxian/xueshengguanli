...
namespace MVC
{
    internal class PagerBuilder
    {
            ...
            AjaxOptions ajaxOptions, IDictionary<string, object> htmlAttributes)
        {
            ...
            {
                if (routeValues != null && routeValues["action"] != null)
                {
                    actionName = routeValues["action"].ToString();
                    html.ViewContext.RouteData.Values["action"] = actionName;
                }
                else
                    if (ajax != null && ajax.ViewContext.RouteData.Values["action"] != null)
            ...
            {
                if (routeValues != null && routeValues["controller"] != null)
                {
                    controllerName = routeValues["controller"].ToString();
                    html.ViewContext.RouteData.Values["controller"] = controllerName;
                }
                else if (ajax != null)
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            {
                if (key != _pagerOptions.PageIndexParameterName && key != "_") //$.ajax调用自动附加"_"参数
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li class=\"disabled\"><a>{0}</a></li>", item.Text));
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? String.Format("<li class=\"active\"><a>" + item.Text + "</a></li>")
