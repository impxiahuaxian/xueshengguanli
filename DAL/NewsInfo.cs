...
namespace Maticsoft.DAL
{
	...
	public partial class NewsInfo
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "NewsInfo"); 
		}
		...
		public int Add(Maticsoft.Model.NewsInfo model)
		{
			...
			strSql.Append("Title,Time,NewsContent,PicID,UserID,Str1,Str2)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@PicID", SqlDbType.NVarChar,50),
		...
		public Maticsoft.Model.NewsInfo GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Title,Time,NewsContent,PicID,UserID,Str1,Str2 from NewsInfo ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.NewsInfo DataRowToModel(DataRow row)
		{
			Maticsoft.Model.NewsInfo model=new Maticsoft.Model.NewsInfo();
			if (row != null)
			{
				...
				if(row["PicID"]!=null)
				{
					model.PicID=row["PicID"].ToString();
				}
				if(row["UserID"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,Title,Time,NewsContent,PicID,UserID,Str1,Str2 ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,Title,Time,NewsContent,PicID,UserID,Str1,Str2 ");
			strSql.Append(" FROM NewsInfo ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
